...
def process_data(data_dir, data_identifier,winSize=30):
    ...
    # keep first keep_p% data
#    if keep_p < 1:
#        id_cycle_lengths = train_01_raw.groupby('id')['cycle'].count()
#        id_rows_to_keep = (id_cycle_lengths * keep_p).astype(int)
#        
#        def filter_rows(group):
#            return group.head(id_rows_to_keep[group.name])
#        
#        train_01_raw = df.groupby('id', group_keys=False, as_index=False).apply(filter_rows)
    ...
    #valid_data[:, :, 0] = scaler.transform(valid_data[:, :, 0])#normalize
...
def get_train_valid(data,window_size,max_RUL):
    ...
    train_size = int(0.99 *num_engines)
...
def plotDataDistribution(data_identifier, data, labels):
    sensors = []
    plt.figure(figsize=(30, 10))
    new_data = np.reshape(data, (-1, 14))
    ...
    plt.subplot(3, 10, new_data.shape[1]+1)
    snsplot = sns.distplot(labels)
    plt.tight_layout()
    
    scatter_fig = snsplot.get_figure()
    scatter_fig.savefig(data_identifier+'.png')
    
    return sensors
    
# --------------main-------------------
    
data_dir = '/home/user/Documents/incomplete_DA/data/CMAPSSData/' #'./CMAPSSData/'
winSize = 30
data_dict = {}
for data_identifier in ['FD001', 'FD002', 'FD003','FD004']:
    ...
    print(train_data)
    print(data_identifier, train_data.shape)
    plotDataDistribution(data_identifier, train_data, train_labels)
    #data_dict[data_identifier] = [train_data, valid_data, test_data, train_labels, valid_labels, test_labels]
    data_dict[data_identifier] = [train_data, test_data, train_labels, test_labels]
    
    train_unique, train_counts = np.unique(train_labels, return_counts=True)
    test_unique, test_counts = np.unique(test_labels, return_counts=True)
    #print('train', np.asarray((train_unique, train_counts)).T)
    #print('test', np.asarray((test_unique, test_counts)).T)
    
#torch.save(data_dict, './processed_data/cmapps_train_test_cross_domain.pt')